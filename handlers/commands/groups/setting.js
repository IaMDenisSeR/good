...
module.exports = {
    ...
    exec: async (m, client, { body, url, prefix, args, cmd, flags }) => {
        try {
            ...
            if (/^lock|closed|close$/.test(args[0])) {
                ...
                if (_text.length < 1) return m.reply(`Change name group, example: ${prefix + cmd} ${args[0]} ${package.name}`)
            ...
            } else if (/^desc|description|rules$/) {
                if (_text.length < 1) return m.reply(`Change description group, example: ${prefix + cmd} ${args[0]} ssstt... dilarang mengontol wkwkwk!`)
                await client.groupUpdateDescription(m.chat, _text)
            } else {
                m.reply(`Enter options, Ex: ${prefix + cmd} lock => close group\n\nlist option:\n-${this.opts.join('\n-')}`)
            }
        } catch (error) {
            m.reply('```' + error + '```')
            console.log(error);
